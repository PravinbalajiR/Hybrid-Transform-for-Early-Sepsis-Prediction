...
\title{\textbf{High Discrimination Does Not Guarantee Clinical Utility:\\Cross-Hospital Utility Decomposition for Temporal Sepsis Early Warning}}
...
The infeasible \texttt{GROUND\_TRUTH\_ORACLE\_CEILING} was positive ($\mathbf{+0.826246}, 95\%\text{ CI: }[+0.806653, +0.844781]$), indicating that the scoring metric itself does not render positive utility impossible. Dense 2D policy sweeps revealed that even under optimal hindsight thresholding and alert suppression, the global policy ceiling remained strictly negative (\texttt{HINDSIGHT\_GRID\_SCORE\_POLICY\_CEILING} = $-0.198307, 95\%\text{ CI: }[-0.218529, -0.178330]$).
