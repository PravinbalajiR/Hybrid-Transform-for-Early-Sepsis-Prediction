"""
audit_m3_tap_phase2.py
----------------------
Standalone Audit Script for M3-TAP Phase 2.
Verifies all 11 scientific rules and output integrity:
1. M3 frozen checkpoint SHA256 unchanged.
2. Test prediction NPZ SHA256 unchanged.
3. All required output CSV/JSON/MD files exist and are non-empty.
...
def main():
    print("=" * 80)
    print("   M3-TAP PHASE 2 — STANDALONE AUDIT VERIFICATION")
    print("=" * 80)
    ...
    required_files = [
        RESULTS_DIR / "M3_TAP_PHASE2_POLICY_SWEEP.csv",
        RESULTS_DIR / "M3_TAP_PHASE2_TOP_POLICIES.csv",
        RESULTS_DIR / "M3_TAP_PHASE2_VALIDATION_CI.csv",
        RESULTS_DIR / "M3_TAP_PHASE2_FROZEN_POLICY.json",
        RESULTS_DIR / "M3_TAP_PHASE2_TEST_RESULTS.csv",
        RESULTS_DIR / "M3_TAP_PHASE2_UTILITY_DECOMPOSITION.csv",
        REPORTS_DIR / "M3_TAP_PHASE2_AUDIT_REPORT.md",
    ]
    ...
    if not all_files_exist:
        print("   CRITICAL ERROR: One or more required output files missing or empty!")
        sys.exit(1)

    # 3. Scorer Verification Audit
    test_res_df = pd.read_csv(RESULTS_DIR / "M3_TAP_PHASE2_TEST_RESULTS.csv")
    decomp_df = pd.read_csv(RESULTS_DIR / "M3_TAP_PHASE2_UTILITY_DECOMPOSITION.csv")

    off_u = float(test_res_df.iloc[0]["test_utility"])
    dec_u = float(decomp_df.iloc[0]["normalized_utility"])
    diff = abs(off_u - dec_u)

    print("\n3. Official Scorer vs Independent Decomposition Check:")
    print(f"   Official Test Utility      : {off_u:+.6f}")
    print(f"   Decomposition Utility     : {dec_u:+.6f}")
    ...
    print("\n" + "=" * 80)
    print("   ALL PHASE 2 AUDIT CHECKS PASSED SUCCESSFULLY — SCIENTIFICALLY VALID")
    print("=" * 80)
